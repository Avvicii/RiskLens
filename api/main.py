...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        state.threshold     = state.meta.get("optimal_threshold", 0.5)
        ...
        print(f"[API] ✓ Loaded model | {len(state.feature_names)} features | threshold={state.threshold:.3f}")
...
class LoanFeatures(BaseModel):
    ...
    grade:                  float = Field(2,      description="LC grade: A=0,B=1,C=2,D=3,E=4,F=5,G=6")
    emp_length:             float = Field(5,      description="Employment years: 0=<1yr … 10=10+")
    # home_ownership and purpose are one-hot — pass 0/1 for each column
    home_ownership_MORTGAGE: float = Field(0, description="1 if MORTGAGE else 0")
    home_ownership_OWN:      float = Field(0, description="1 if OWN else 0")
    home_ownership_RENT:     float = Field(1, description="1 if RENT else 0")
    purpose_debt_consolidation: float = Field(0, description="1 if debt consolidation")
    purpose_credit_card:     float = Field(0, description="1 if credit card payoff")
    purpose_home_improvement: float = Field(0, description="1 if home improvement")
    purpose_other:           float = Field(0, description="1 if other purpose")
...
def predict_single(features: LoanFeatures) -> PredictResponse:
    ...
    threshold = state.threshold if state.threshold else 0.5
...
def model_info():
    ...
    return {**state.meta, "active_threshold": state.threshold}
